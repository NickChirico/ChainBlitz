...
public class ScreenIndicator : MonoBehaviour
{
	...
	//The icon. Preferably an arrow pointing upwards.
	public float iconSize = 50f;
	[HideInInspector]
	public GUIStyle gooey;
	//GUIStyle to make the box around the icon invisible. Public so that everything has the default stats.
	Vector2 indRange;
	float scaleRes = Screen.width / 500;
	//The width of the screen divided by 500. Will make the GUI automatically
	//scale with varying resolutions.
	Camera cam;
	bool visible = true;
	//Whether or not the object is visible in the camera.

	bool spawnerIsEnabled = true;
	Spawner spawner;
	...
	void Update()
	{
		spawnerIsEnabled = spawner.isEnabled;
	}

	void OnGUI ()
	{
		if (!visible && spawnerIsEnabled)
		{
			Vector3 dir = this.transform.position - cam.transform.position;
